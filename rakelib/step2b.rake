...
file 'tmp/step2b' => step2b_sources do |t|
  pid = fork do
    ...
    tracefp = open("| gsort --parallel=#{PROCS} -S 40% -t '\t' -k 1,1 | pigz -c > #{t.name}.trace", 'w')
    infp = open("| unpigz -c #{t.sources.join(' ')} | gsort --parallel=#{PROCS} -S 40% -t '\t' -k 4,4 -k 3,3r")
    preseq = ''
    preacc = ''
    # out = ''
    # trace = ''
    while line = infp.gets
      ...
      if preseq != seq
        ...
        # end
        begin
          outfp.write_nonblock("#{bc}\t#{acc}\t#{qv}\t#{seq}\n")
        rescue IO::WaitWritable
          IO.select(nil, [outfp])
          retry
        end
      ...
      # end
      begin
        tracefp.write_nonblock("#{acc}\t#{preacc}\n")
      rescue IO::WaitWritable
        IO.select(nil, [tracefp])
        retry
      end
